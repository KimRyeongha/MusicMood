...
| 2026.06.13 | 1.1.0 | 플레이리스트 삭제 기능 다이어그램 추가 및 갱신 | 김령하 |
...
* **회원 관리 기능:** 사용자는 아이디, 비밀번호, 닉네임을 통해 가입 및 로그인을 할 수 있으며, 마이페이지를 통해 언제든 개인정보를 수정하거나 본인이 등록한 곡을 직접 삭제하여 플레이리스트를 깔끔하게 관리할 수 있다.
...
| **MusicController** | 사용자의 새로운 음악 저장 요청, 전체 소셜 음악 목록 및 개인 보관함 목록 조회 요청, 음악에 대한 좋아요 토글 및 등록한 음악 삭제 요청 등 음악 서비스 전반의 API 라우팅을 담당하는 컨트롤러이다. |
...
| **MusicRepository** | 사용자가 플랫폼에 등록한 유튜브 음악의 메타데이터(Video ID, 제목, 태그, 등록자 정보 등)를 MUSIC 테이블에 반영하고 조건별로 데이터를 가져오거나 삭제하는 인터페이스 역할을 한다. |
...
AdminController는 복합적인 지표를 완성하기 위해 여러 Repository를 연속적으로 호출한다. 
...
### 3.7. 플레이리스트 음악 삭제 흐름
사용자가 마이페이지에서 자신이 등록한 특정 음악을 삭제할 때 발생하는 흐름이다. 
프론트엔드에서 DELETE 요청을 보내면, 컨트롤러에서 해당 곡이 존재하는지(findById), 그리고 현재 로그인한 사용자가 올린 곡이 맞는지 권한을 확인한 뒤 데이터베이스에서 안전하게 삭제하고 성공 응답을 반환한다.
...
sequenceDiagram
    ...
    Client->>MusicController: DELETE /api/music/delete/{musicId}?loginId={loginId}
    MusicController->>MusicRepository: findById(musicId)
    alt 곡 존재 및 권한 일치
        MusicRepository-->>MusicController: Optional<Music>
        MusicController->>MusicRepository: delete(music)
        MusicRepository-->>MusicController: void
        MusicController-->>Client: ResponseEntity.ok("삭제 완료")
    else 권한 불일치 또는 곡 없음
        MusicRepository-->>MusicController: Optional.empty() 또는 RuntimeException
        MusicController-->>Client: ResponseEntity.badRequest / 500 에러
    end
```

## 4. State Machine Diagram

M3 애플리케이션에 처음 접속하는 순간부터 음악 탐색, 필터링, 새로운 곡 저장, 좋아요 반영, 음악 삭제 그리고 관리자 화면 확인까지 사용자가 서비스 내에서 경험하게 되는 전반적인 상태 변화와 논리적인 흐름을 시각화한 다이어그램이다.
...
stateDiagram-v2
    ...
    LoggedOut --> BrowsingMusic: 홈 화면 접속 (비로그인 둘러보기)
    ...
    state MainService {
        ...
        state MusicOperations {
            ...
            BrowsingMusic --> Authentication: 음악 추가 / 좋아요 시도 (비로그인 시)
            
            BrowsingMusic --> FilteringMusic: 감성 태그 필터 적용
            FilteringMusic --> BrowsingMusic: 필터링 완료
            
            BrowsingMusic --> AddingNewMusic: 음악 추가 시작 (로그인됨)
        ...
        state UserManagement {
            ...
            ViewingMyPage --> ConfirmingDeleteModal: 삭제 버튼(X) 클릭
            ConfirmingDeleteModal --> ViewingMyPage: 취소 클릭
            ConfirmingDeleteModal --> DeletingMusic: 삭제하기 클릭
            DeletingMusic --> ShowingSuccessModal: 통신 성공
            ShowingSuccessModal --> ViewingMyPage: 확인 클릭
        }
...
#### 1) 초기 상태 (Entry Point)
* **LoggedOut**: 애플리케이션에 접속한 직후의 초기 상태이자 비로그인 상태이다. 홈 화면에 진입하여 타인의 플레이리스트를 자유롭게 둘러볼 수 있으나, 음악 추가나 좋아요 등의 주요 액션을 시도할 경우 로그인 화면으로 리다이렉트된다.
...
* **ConfirmingDeleteModal**: 삭제 버튼을 클릭했을 때 사용자의 실수를 방지하기 위해 띄우는 재확인 모달 창 상태이다.
* **DeletingMusic**:모달에서 확정 시, 백엔드 서버와 통신하여 데이터베이스에서 영구적으로 곡을 지우는 런타임 상태이다.
* **ShowingSuccessModal**:삭제 통신이 성공적으로 마무리된 후, 사용자에게 완료를 알리는 최종 모달 창 상태이다.
