...
| 2026.06.13 | 1.1.0 | 플레이리스트 삭제 기능 추가 및 이미지 수정 | 김령하 |
...
M3는 이용자가 직접 유튜브 링크와 감성 태그를 등록하고, 태그 필터로 그날의 무드에 맞는 음악을 즉시 찾고, 감상할 수 있는 서비스이다. 주요 기능으로는 회원 관리, 음악 등록, 태그 필터링, 소셜 플레이리스트 공유, 좋아요 기능, 관리자 공식 추천 플레이리스트 관리, 시스템 통계 대시보드가 있다.
...
| :--- | :--- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 이용자가 유튜브 링크와 감성 태그를 입력하여 음악 등록 요청을 한다. |
| Scope | My Mood, My Music (M3) |
| Level | User level |
| Author | |
| Last Update | 2026.05 |
...
| 1 | 홈 화면 상단의 음악 추가 영역에서 유튜브 링크를 입력한다. |
...
| **EXTENSION SCENARIOS** | |
| **Step** | **Branching Action** |
| 1 | 1a. 링크를 입력하지 않은 경우 → '링크를 입력해주세요!' 출력. |
...
| Secondary Actors | System, YouTubeService |
| Preconditions | 로그인이 완료된 상태이다. |
| Trigger | 홈 화면에서 유튜브 링 입력, 태그를 선택한 후 '저장' 버튼을 클릭했을 때. |
| Success Post Condition | 유튜브 API에서 제목·썸네일을 받아와 Music 테이블에 저장되고 플레이리스트가 즉시 갱신된다. |
| Failed Post Condition | 유효하지 않은 Video ID이거나 API 오류 발생 시 저장에 실패하고 오류 메시지를 출력한다. |
| **MAIN SUCCESS SCENARIO** | |
| **Step** | **Action** |
...
| **EXTENSION SCENARIOS** | |
| **Step** | **Branching Action** |
| 1 | 1a. 유튜브 링크를 아예 입력하지 않거나, 유효한 형태의 링크가 아닌 경우 → '유효한 링크를 입력해주세요!' 알림 출력 후 저장 중단. |
| 2 | 2a. 감성 태그를 '직접입력'으로 선택했으나 텍스트를 작성하지 않은 경우 → '직접 입력할 감성 태그를 적어주세요!' 알림 출력 후 저장 중단. |
| 4 | 4a. YouTube API에서 영상을 찾을 수 없거나, 서버/네트워크 오류로 인해 처리가 거부된 경우 → catch문으로 넘어가 '저장에 실패했습니다.' 알림 출력. |
| **RELATED INFORMATION** | |
| Performance | ≦ 3 Seconds |
| Frequency | Variable |
| Concurrency | None |
...
| 5 |삭제 버튼을 클릭하면 확인 모달창이 나타나며, 확정 시 해당 곡이 삭제되고 목록이 즉시 갱신된다. |
| **EXTENSION SCENARIOS** | |
| **Step** | **Branching Action** |
| 2 | 2a. 등록한 곡이 없는 경우 → 빈 목록 표시 (0곡). |
| 4 | 4a. 선택한 태그에 해당하는 곡이 없는 경우 → '해당 태그에 등록한 내 음악이 없습니다.' 출력. |
| 5 | 5a. 삭제 확인 모달에서 '취소'를 누르면 모달이 닫히고 화면이 유지된다. |
| **RELATED INFORMATION** | |
| Performance | ≦ 3 Seconds |
| Frequency | Variable |
| Concurrency | None |
...
| :--- | :--- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 이용자가 입력한 링크의 제목과 썸네일 정보를 확보하기 위해 YouTube Data API v3에 요청한다. |
| Scope | My Mood, My Music (M3) |
| Level | User level |
| Author | |
| Last Update | 2026.05 |
...
| Preconditions | 이용자가 유효한 유튜브 링크를 입력하여 Register Music을 실행한 상태이다. |
...
| Secondary Actors | System, YouTubeService |
| Preconditions | 관리자(admin) 계정으로 로그인한 상태이다. |
| Trigger | 홈 화면의 음악 추가 영역에서 링크와 태그를 입력한 후 '저장' 버튼을 클릭했을 때. |
...
| 1 | 홈 화면의 음악 추가 영역에서 유튜브 링크와 감성 태그를 입력한다. |
| 2 | '저장' 버튼을 클릭한다. |
...
로그인 후 이용자가 가장 먼저 보게 되는 화면이다.<br> 상단에는 마이페이지, 로그아웃 버튼이 있고 관리자에게는 '시스템 통계' 버튼이 추가로 표시된다.<br> 음악 추가 영역에서 유튜브 링크와 감성 태그를 선택 또는 직접 입력하여 저장할 수 있다.<br> 태그 필터 버튼으로 관리자 추천 플레이리스트를 필터링할 수 있다.<br> 화면 하단에는 'M3 추천 플레이리스트'와 '모두의 플레이리스트'가 분리 표시된다.<br> 각 음악 카드에서 하트를 클릭하여 좋아요를 토글할 수 있다.


### 4.4. MyPage 화면
![MyPage](./src/images/mypage.png)

이용자가 개인정보를 관리하고 자신의 플레이리스트를 모아볼 수 있는 화면이다.<br>닉네임 변경 폼과 비밀번호 변경 폼이 나란히 표시된다. <br>하단에는 내가 올린 플레이리스트와 내가 찜한 플레이리스트가 각각 그리드로 표시된다. <br>자신이 등록한 곡을 삭제할 수 있다. <br>찜한 목록에서는 하트를 클릭하여 즉시 취소할 수 있다. <br>관리자가 올린 공식 추천곡은 '공식 추천곡' 배지로, 일반 이용자 곡은 '올린이 - 닉네임' 형식으로 표시된다.
...
| **Video ID** | 유튜브 영상 URL에 포함된 11자리의 고유 식별자. 사용자가 입력한 링크에서 시스템이 자동으로 추출하여 사용한다. |
